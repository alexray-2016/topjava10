# Онлайн проекта <a href="https://github.com/JavaWebinar/topjava10">Topjava</a>
...
> - Ошибка в патче `10_15_csrf.patch`: CSFR должен выставляется в `makeEditable()` для всех запросов.
> - Удалил `<%@ page session="false` в JSP: не отображаются ошибки авторизации, которые находятся в сессии, отключенной от JSP.
> - Небольшие правки кода
 
#### Apply 11_0_2_fix_refactoring
> - Поправил локализацию
> - Перенес `DateTimeFormatters` в пакет web (он доступен только на слое контроллеров)
> - Сделал `DATAJPA` дефолтной реализацией (используется по умолчанию в Spring Boot, самая передовая)
> - Удалил лишний код, мелкая правка 
...
> - Сделал общий интерфейс валидации `View.ValidatedRestUI` для Ajax/REST (`Default` для сохранения в базу) 
>   - `Meal` валидируется по разному в контроллерах и при сохранении в базу (`@NotNull Meal.user`), мы делаем группы и используем `@Validated`. 
>   - `User` валидируется одинаково, поэтому групп нет и используем `@Valid`
> - Сделал тесты с валидацией через [JSONassert](https://github.com/skyscreamer/JSONassert), можно рассматривать как легкая замена нашим матчерам
...
> - Сделал проверку в тестах [через `jsonPath`](https://www.petrikainulainen.net/programming/spring-framework/integration-testing-of-spring-mvc-applications-write-clean-assertions-with-jsonpath/)
> - `HttpStatus` при ошибке дублирования возвращаю не через `@ResponseStatus` а через `ResponseEntity` (так его можно менять)
...
- [В JSP: the authentication Tag](http://docs.spring.io/spring-security/site/docs/current/reference/html/taglibs.html#the-authentication-tag)
  - авторизованный пользователь через tag `authentication` доступен в JSP, интерсептор становится не нужным
...
- Сделать и внести в свое портфолио <a href="https://github.com/JavaWebinar/topjava10/blob/doc/doc/graduation.md#graduation-project">Graduation project</a>
  - Для участников с проверкой выпускной проект принимаю на ревью до 06.07 (участникам <a href="http://javaops.ru/reg/masterjava">Masterjava</a> + 3 месяца). 
